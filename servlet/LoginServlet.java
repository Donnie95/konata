...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// Forward to /WEB-INF/views/loginView.jsp users cannot access directly into JSP
		// pages placed in WEB-INF
		RequestDispatcher dispatcher = this.getServletContext().getRequestDispatcher("/WEB-INF/views/loginView.jsp");

		dispatcher.forward(request, response);
	}

	// When the user enters userName & password, and click Submit this method will
	// be executed
	@Override
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// If no error store user information in session and redirect to Ambients page
			...
			if (name.matches("[0-9a-zA-Z_]+") && chiave.matches("[0-9a-zA-Z_]+") && chiave != null && name != null)
